...
const AddImagesProduct = () => {
    ...
    async function handleUploadImage(data) {
        try {
            ...
            const responseAtt = await api.put(`store?id=${dataAuth.id}`);
    ...
    async function handleCancel() {
        Alert.alert('Cancelar', 'Deseja realmente excluir o produto?', [
            {
                text: 'Não',
                onPress: () => {
                    return;
    ...
    async function handleMarkMainFile(id) {
        ...
        const updateFileMark = await api.put(
            `mainProduct?id=${id}&product=${idProduct}`
        );
